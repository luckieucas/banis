...
OUTPUT_ROOT="/projects/weilab/liupeng/runs/banis/mitoem2_graph_repair_stage1/mitonet_v1_zero_shot/test_deterministic_v100_seed0"
...
for GRAPH in "${GRAPHS[@]}"; do
  ...
  JOB=$(sbatch --parsable --partition "${PARTITION}" --nodelist g002 \
    --gres gpu:v100:1 --cpus-per-task 8 \
    --mem "${MEMORY}" --time "${TIME_LIMIT}" --job-name "mitonet_${CASE}" \
...
SUMMARY_JOB=$(sbatch --parsable --partition short --cpus-per-task 1 --mem 8G --time 01:00:00 \
  --dependency "afterok:${DEPENDENCY}" --job-name mitonet_test_summary \
